...
var BackendProcess = function(){
    ...
    this.spawn = function (root, backendDirectory, entryPoint, port) {
        ...
        if (this.isPacked(exe_script) === true) {
            ...
            if (process.platform === 'win32') {
              this.pyProc = child_process.spawn(filename + '.exe', [port], {detached: false});
          } else {
              this.pyProc = child_process.spawn(filename, [port], {detached: false});
          }
        } else {
            try {
        ...
        setInterval(function () {
            console.log('Resume backend stdio...');
            this.pyProc.stdin.resume();
            this.pyProc.stdout.resume();
        }.bind(this), 3000);

        this.pyProc.stdout.on('data', (data) => {
          data = data.toString('utf8');// buffer to string
          re = /SERVING .* ON .*:([0-9]{1,5})/g;
          console.log('[Meson] <backend::stdout>');
          console.log(data);
          console.log('[Meson] </backend::stdout>');
