...
var FrontendFrontalController = function (frontendKernel, operationRouter){
    ...
    this.exit = function (reason) {
        for(var futureId in this.operationResultFutures) {
            let future =  this.operationResultFutures[futureId];
            if (reason === undefined) {
...
var FrontendApplication = function (app_id, server_secret, client_secret) {
    ...
    this.bindSecureLayer = function (pipeline) {
      var sessionKey = pipeline.sessionKey;
      var sessionIV  = pipeline.sessionIV;
    ...
    this.waitForStarted = new Promise(function (resolve, reject) {
        this.started = resolve;
        this.startFailed = reject;
    }.bind(this));

    this.waitForExit = new Promise(function (resolve) {
    ...
    this.start = function (addr, port) {
        ...
        return co.wrap(function* (frontend, websocket) {
            try {
              ...
              var pipeline = yield frontend.buildPipeline(websocket);
              pipeline.on('close', frontend.onPipelineClose);

              var security  = frontend.bindSecureLayer(pipeline);
              var protocol = frontend.bindProtocolLayer(security);
              var operationRouter = frontend.bindOperationRouter(protocol);
              // Frontend Kernel
              var kernel = frontend.buildKernel();
              // Frontend Frontal Controller
              var frontal = frontend.buildFrontalController(kernel, operationRouter);
              // Set the started status
              frontend.status = frontend.STATUS_STARTED;
              frontend.started();
            } catch (e) {
              frontend.startFailed();
              console.error(e);
              throw e;
            }
        })(this, ws);
    ...
    this.exit = function (reason) {
        if (reason === undefined) {
            reason = '[Meson] Backend closed without any reason.';
        }
    ...
    this.onPipelineClose = function (pipeline, reason) {
      console.error('Pipeline closed!');
      this.exit(reason);
    }.bind(this);
};
